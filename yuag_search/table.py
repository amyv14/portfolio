...
class Table:
    ...
    def to_html(self) -> str:
        """Converts the table to an HTML table string."""
        html = '<table border="1" style="border-collapse: collapse; margin: 10px 0;">\n'
        # Header row
        html += '<tr>' + ''.join(f'<th style="padding: 5px; background-color: #f2f2f2;">{header}</th>' for header in self._column_names) + '</tr>\n'
        # Data rows
        for row in self._data:
            html += '<tr>' + ''.join(f'<td style="padding: 5px;">{cell}</td>' for cell in row) + '</tr>\n'
        html += '</table>'
        return html
